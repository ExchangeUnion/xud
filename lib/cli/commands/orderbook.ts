...
export const createOrderbook = (
  orders: ListOrdersResponse.AsObject,
  precision: number
) => {
...
export const createOrderbookSide = (
  orders: Order.AsObject[],
  precision = 5
) => {
...
export const describe =
  'display the order book, with orders aggregated per price point';
...
export const builder = (argv: Argv) =>
  argv
    ...
    .example(
      '$0 orderbook LTC/BTC 2',
      'display the LTC/BTC order book with 2 decimal precision'
    )
    .example(
...
const displayJson = (
  ...
  orders.ordersMap.forEach(tradingPair => {
    const buy = createOrderbookSide(
      tradingPair[1].buyOrdersList,
      argv.precision
    );
    buy.forEach(quantityInSatoshisPerCoin);
    const sell = createOrderbookSide(
      tradingPair[1].sellOrdersList,
      argv.precision
    );
...
export const handler = async (argv: Arguments<any>) => {
  ...
  (await loadXudClient(argv)).listOrders(
    request,
    callback(argv, displayTables, displayJson)
  );
};
