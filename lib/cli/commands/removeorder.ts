...
export const builder = (argv: Argv) =>
  argv
    ...
    .option('quantity', {
      type: 'number',
      describe:
        'quantity to remove; if unspecified the entire order is removed',
    })
    .example(
      '$0 removeorder 79d2cd30-8a26-11ea-90cf-439fb244cf44',
      'remove an order by id'
    )
    .example(
      '$0 removeorder 79d2cd30-8a26-11ea-90cf-439fb244cf44 0.1',
      'remove 0.1 quantity from an order by id'
    );

const displayOutput = (
  orderId: string,
  removeOrderResponse: RemoveOrderResponse.AsObject
) => {
  ...
  if (quantityOnHold === 0 && remainingQuantity === 0) {
    ...
    if (remainingQuantity) {
      message += `, ${satsToCoinsStr(
        remainingQuantity
      )} ${removedCurrency} remaining quantity`;
    }
    if (quantityOnHold) {
      message += `, ${satsToCoinsStr(
        quantityOnHold
      )} ${removedCurrency} was on hold`;
    }
    console.log(message);
  }
};
...
export const handler = async (argv: Arguments<any>) => {
  ...
  (await loadXudClient(argv)).removeOrder(
    request,
    callback(argv, displayOutput.bind(undefined, argv.order_id))
  );
};
