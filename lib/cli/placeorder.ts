...
export const placeOrderBuilder = (argv: Argv, side: OrderSide) => {
  ...
  argv
    ...
    .example(
      `$0 ${command} 3 LTC/BTC mkt`,
      `place a market order to ${command} 3 LTC for BTC`
    )
    .example(
      `$0 ${command} 10 ZRX/GNT market`,
      `place a market order to ${command} 10 ZRX for GNT`
    );
};
