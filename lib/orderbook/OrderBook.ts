...
class OrderBook extends EventEmitter {
  ...
  public init = async () => {
    ...
    pairs.forEach(pair => {
      ...
      this.tradingPairs.set(
        pair.id,
        new TradingPair(this.logger, pair.id, this.nomatching)
      );
    });
  ...
  public addPair = async (pair: Pair) => {
    ...
    this.tradingPairs.set(
      pairInstance.id,
      new TradingPair(this.logger, pairInstance.id, this.nomatching)
    );
  ...
  public addCurrency = async (currency: CurrencyFactory) => {
    ...
    if (
      currency.swapClient === SwapClientType.Raiden &&
  ...
  private placeOrder = async ({
    ...
    const outboundSwapClient = this.swaps.swapClientManager.get(
      outboundCurrency
    );
    ...
    if (!this.nobalancechecks) {
      ...
      const totalOutboundAmount = outboundSwapClient.totalOutboundAmount(
        outboundCurrency
      );
      if (outboundAmount > totalOutboundAmount) {
        throw errors.INSUFFICIENT_OUTBOUND_BALANCE(
          outboundCurrency,
          outboundAmount,
          totalOutboundAmount
        );
  ...
  private verifyPeerPairs = async (peer: Peer) => {
    ...
    currenciesToVerify.forEach((swappable, currency) => {
      if (swappable || this.testing) {
        // always activate currencies if in debug/testing mode
