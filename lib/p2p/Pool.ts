...
const minCompatibleVersion: string = require('../../package.json')
...
interface Pool {
  ...
  on(
    ...
    listener: (orderInvalidation: OrderPortion, peer: string) => void
  ): this;
...
class Pool extends EventEmitter {
    ...
    switch (packet.type) {
      case PacketType.Order: {
        ...
        if (peer.isPairActive(incomingOrder.pairId)) {
          ...
          this.logger.debug(
            `received order ${incomingOrder.id} for deactivated trading pair`
          );
        }
        break;
      }
  ...
  private bindPeer = (peer: Peer) => {
    ...
    peer.on('reputation', async event => {
      this.logger.debug(
        `Peer (${peer.label}): reputation event: ${ReputationEvent[event]}`
      );
      if (peer.nodePubKey) {
        if (
          this.testing &&
