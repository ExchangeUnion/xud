...
const errorCodes = {
  ...
  NO_INVOICE_SUPPORT: codesPrefix.concat('.4'),
};
...
const errors = {
  ...
  NO_INVOICE_SUPPORT: (currency: string) => ({
    message: `lnd-${currency} has no invoice support`,
    code: errorCodes.NO_INVOICE_SUPPORT,
  }),
};
